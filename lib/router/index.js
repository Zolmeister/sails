module.exports = function(sails) {
	...
	function Router(options) {
      ...
      // required here because of dynamic environment variable loading
      expressApp = require('express')();
			...
			// Bind configured 404 and 500 behaviors
			sails.on('router:done', function () {
				
				// 400 handler
				sails.router.app.use(sails.config[404]);

				// Error handler
				sails.router.app.use(sails.config[500]);
			});
		...
		Router.prototype.route = function(req, res, cb) {
			...
			expressApp.router( req, res, function noMatch(err) {
				...
				if (err) {
					...
					sails.log.error(err);
					res.send('Server error :: ' + util.inspect(err), 500);
					return;
				}
